...
Come strumento utilizzeremo il linguaggio Haskell, un linguaggio di programmazione che rispetto ai più comuni linguaggi basati su paradigmi imperativi e/o ad oggetti segue una linea di pensiero più vicina alla matematica. Questa è, programmazione funzionale, un tipo di programmazione sviluppata dagli stessi logici, infatti il linguaggio che adotteremo Haskell il nome si riferisce al matematico e logico Haskell Curry.


Utilizzeremo il modulo Utils, dando per scontato il funzionamento delle varie funzioni contenute essendo intuitivo. In aggiunta anticipiamo come debba essere il tipo di un Parser, dandone una leggera spiegazione.
...
\end{lstlisting}
Dunque l'istanza di un Parser a, è un "oggetto" il cui scopo è elaborare una stringa String -> [risultato] e restituire un tipo a, ed una stringa "coda" risultato. Allora la funzione parse non fa altro che applicare ad un Parser a l'input inp. Dove convenzionalmente utilizziamo una lista vuota per associare il fallimento del parsing, altrimenti vi sarà una lista contenente una tupla: (a,String) ovvero la restituzione del tipo a ed nuova stringa.
...
\end{lstlisting}
\begin{lstlisting}[style=ghcistyle]
@ghci>@ putStrln "ciao, come state"
ciao, come state
@ghci>@ 
\end{lstlisting}
